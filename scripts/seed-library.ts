...
async function run() {
  console.log('Seeding Library Master Catalog Templates...');
  try {
    await signInWithEmailAndPassword(auth, 'admin@demo.com', 'admin123');
    const items = catalogTemplates.library || [];
    console.log(`Loaded ${items.length} Library templates. Uploading to productTemplates...`);
