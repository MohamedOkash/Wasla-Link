...
async function run() {
  console.log('Seeding Grocery Master Catalog Templates...');
  try {
    await signInWithEmailAndPassword(auth, 'admin@demo.com', 'admin123');
    const items = catalogTemplates.grocery || [];
    console.log(`Loaded ${items.length} Grocery templates. Uploading to productTemplates...`);
    
    for (const item of items) {
      await setDoc(doc(db, 'productTemplates', item.id), item);
    }
    
    console.log('Grocery seeding completed successfully!');
    process.exit(0);
  } catch (err) {
    console.error('Grocery seeding failed:', err);
    process.exit(1);
  }
}
