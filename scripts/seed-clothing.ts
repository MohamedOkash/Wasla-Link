...
async function run() {
  console.log('Seeding Clothing Master Catalog Templates...');
  try {
    await signInWithEmailAndPassword(auth, 'admin@demo.com', 'admin123');
    const items = catalogTemplates.clothing || [];
    console.log(`Loaded ${items.length} Clothing templates. Uploading to productTemplates...`);
    
    for (const item of items) {
      await setDoc(doc(db, 'productTemplates', item.id), item);
    }
    
    console.log('Clothing seeding completed successfully!');
    process.exit(0);
  } catch (err) {
    console.error('Clothing seeding failed:', err);
    process.exit(1);
  }
}
