...
async function run() {
  console.log('Seeding Electrical Master Catalog Templates...');
  try {
    await signInWithEmailAndPassword(auth, 'admin@demo.com', 'admin123');
    const items = catalogTemplates.electric || [];
    console.log(`Loaded ${items.length} Electrical templates. Uploading to productTemplates...`);
    
    for (const item of items) {
      await setDoc(doc(db, 'productTemplates', item.id), item);
    }
    
    console.log('Electrical seeding completed successfully!');
    process.exit(0);
  } catch (err) {
    console.error('Electrical seeding failed:', err);
    process.exit(1);
  }
}
