...
async function run() {
  console.log('Seeding Mobile Master Catalog Templates...');
  try {
    await signInWithEmailAndPassword(auth, 'admin@demo.com', 'admin123');
    const items = catalogTemplates.mobile || [];
    console.log(`Loaded ${items.length} Mobile templates. Uploading to productTemplates...`);
    
    for (const item of items) {
      await setDoc(doc(db, 'productTemplates', item.id), item);
    }
    
    console.log('Mobile seeding completed successfully!');
    process.exit(0);
  } catch (err) {
    console.error('Mobile seeding failed:', err);
    process.exit(1);
  }
}
